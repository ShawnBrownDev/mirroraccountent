...
export function getNextDueDate(dueDay: number): Date {
  ...
  const dueDateThisMonth = getDueDateForMonth(dueDay, currentMonth, currentYear);
  ...
  return getDueDateForMonth(dueDay, nextMonth, nextYear);
}
...
export function formatDueDate(dueDate: Date): string {
  ...
  if (daysUntil <= 7 && daysUntil > 1) return `Due in ${daysUntil} days`;
